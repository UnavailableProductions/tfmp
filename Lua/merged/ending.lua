function ending(enddataid)
	...
	if (phase == 1) then
		...
		if (timer == 52) then
			...
			if (build == "m") then
				flower.x = tilesize * unit.values[XPOS] * spritedata.values[TILEMULT] - tilesize * 0.25 * spritedata.values[TILEMULT]
				flower.y = tilesize * unit.values[YPOS] * spritedata.values[TILEMULT] - tilesize * 0.5 * spritedata.values[TILEMULT]
			end
		end
		...
		if (timer == 220) then
			...
			for i,unit in pairs(textconverts) do
				local mats2 = {{"blossom",unit.values[ID],unit.values[ID]}}
	...
	if (phase == 2) then
		...
		elseif (timer > 430) and (generaldata.values[TRANSITIONED] == 1) then
			...
			if (build == "m") then
				flower.x = f_tilesize * 16 * spritedata.values[TILEMULT] * spritedata.values[SPRITEMULT] - 64
				flower.y = f_tilesize * 14 * spritedata.values[TILEMULT] * spritedata.values[SPRITEMULT] - 28
			end
			...
			for i,v in ipairs(otherareas) do
				...
				if (status == 1) then
					local flower2id = MF_specialcreate("Flower_center")
					local flower2 = mmf.newObject(flower2id)
					
					flower2.strings[1] = tostring(v.colour[1]) .. "," .. tostring(v.colour[2])
					flower2.strings[2] = "otherarea"
					flower2.values[10] = 2
					flower2.values[8] = 1
					flower2.values[6] = 1
					flower2.x = Xoffset + (v.xpos * tilesize + tilesize * 0.5) * spritedata.values[TILEMULT]
					flower2.y = Yoffset + (v.ypos * tilesize + tilesize * 0.5) * spritedata.values[TILEMULT]
					
					if (build == "m") then
						flower2.x = Xoffset + (v.xpos * f_tilesize + f_tilesize * 0.5) * spritedata.values[TILEMULT] * spritedata.values[SPRITEMULT] - 64
						flower2.y = Yoffset + (v.ypos * f_tilesize + f_tilesize * 0.5) * spritedata.values[TILEMULT] * spritedata.values[SPRITEMULT] - 28
					end
				end
			end
		end
	...
	elseif (phase == 5) then
		...
		if (timer == 100) then
			ending_load(tiles,"keke",-3.75,12.4)
			ending_load(tiles,"skull",3.5,20.4)
			ending_load(tiles,"flag",-3.5,34.5)
			ending_load(tiles,"key",3.5,39)
			ending_load(tiles,"rocket",-3.5,55.1)
		end
	elseif (phase == 6) then
		if (timer == 2) then
			local tiles =
...
function allisdone(enddataid)
	...
	if (phase == 1) then
		if (timer == 350) then
			...
			local ssize = spritedata.values[SPRITESIZE]
			
			MF_setroomoffset(xoffset,yoffset+1024)
			MF_loadbackimage("island_full")
			MF_movebackimage(0,0-480)
			
			if (build == "m") then
				MF_movebackimage(0-12,0)
			else
				MF_movebackimage(0-24,0)
			end
		end
	elseif (phase == 2) then
		local finalpos = 480
		...
		if (build == "m") then
			finalpos = 480 * spritedata.values[SPRITEMULT] + f_tilesize * 2
		end
		
		local xoffset = (screenw - roomsizex * tilesize * spritedata.values[TILEMULT]) * 0.5
		local yoffset = (screenh - roomsizey * tilesize * spritedata.values[TILEMULT]) * 0.5 - finalpos
		...
		if (timer == 200) then
			...
			if (build ~= "m") then
				...
				particles("smoke",14,4+20,10,{0,3},2,1)
			end
		end
		...
		if (timer > 220) and (timer < 420) then
			if (build ~= "m") then
				...
				particles("smoke",14,4+20,2,{0,4},2,1)
			end
		end
		...
		if (timer == 400) then
			...
			if (build ~= "m") then
				...
				blob.y = 62 + Yoffset + 24 + finalpos
			else
				blob.x = 372 + Xoffset + 24 * 2.9
				blob.y = 62 + Yoffset + 24 + finalpos
			end
...
function dointro(introdataid)
				...
				elseif (timer == b.start + b.wait_until_open + b.wait_until_reveal + b.wait_until_position) and b.position then
					for i,unit in ipairs(units) do
						if (unit.strings[UNITNAME] == b.what_to_position) then
							...
							local changecolour = nil
							if (changedata.colour ~= nil) and (tostring(type(changedata.colour)) == "string") then
								changecolour = MF_parsestring(changedata.colour)
							end
							
							local c = changecolour or thisdata.colour
	...
	elseif (phase == 3) then
		...
		for i,unit in ipairs(units) do
			...
			elseif (unitname == "text_stop") or (unitname == "text_push") or (unitname == "text_win") or (unitname == "text_near") or (unitname == "text_power") or (unitname == "text_powered") then
				table.insert(allis, unit)
			end
		end
	...
	elseif (phase == 6) then
		...
		if (timer == 620) then
			...
			MF_store("save",world,"firsttime",tostring(1))
			
			MF_setfile("save",tostring(generaldata2.values[SAVESLOT]) .. "ba.ba")
			
			MF_store("save",world,"intro",tostring(1))
			MF_store("save",world,"firsttime",tostring(1))
			MF_store("save",world,"0level_intro",tostring(1))
